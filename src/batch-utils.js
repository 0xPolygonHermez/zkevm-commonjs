 ...
 * fields: [type | nonce | gasPrice | gasLimit | isDeploy | to | value | dataLen | data | chainId | from | effPer ]
 * bytes:  [ 1   |   8   |    32    |     8    |    1     | 20 |   32  |    3    |  XX  |    8    |  20  |    1   ]
...
function serializeLegacy(tx) {
    ...
    data = Scalar.add(data, Scalar.shl(tx.chainId, offsetBits));
    offsetBits += 64;
...
/**
 * Serialize transaction for the batch
 * fields: [type | nonce | gasPrice | gasLimit | isDeploy | to | value | dataLen | data | from | effPer ]
 * bytes:  [ 1   |   8   |    32    |     8    |    1     | 20 |   32  |    3    |  XX  |  20  |    1   ]
 * @param {Object} tx - transaction object
 * @returns {String} - Serialized tx in hexadecimal string
 */
...
function deserializeLegacy(_serializedTx) {
    ...
    tx.chainId = `0x${serializedTx.slice(offsetChars, offsetChars + 8 * 2)}`;
    offsetChars += 8 * 2;
...
function deserializeChangeL2Block(_serializedTx) {
    ...
    tx.type = parseInt(serializedTx.slice(offsetChars, offsetChars + Constants.TYPE_BYTES * 2), 16);
    offsetChars += Constants.TYPE_BYTES * 2;

    tx.deltaTimestamp = Scalar.fromString(serializedTx.slice(offsetChars, offsetChars + Constants.DELTA_TIMESTAMP_BYTES * 2), 16);
    offsetChars += Constants.DELTA_TIMESTAMP_BYTES * 2;

    tx.indexL1InfoTree = parseInt(serializedTx.slice(offsetChars, offsetChars + Constants.INDEX_L1INFOTREE_BYTES * 2), 16);
