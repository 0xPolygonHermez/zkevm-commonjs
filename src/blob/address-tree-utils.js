...
/**
 * Compute smt key for an address in the address tree
 ...
 */
async function keyAddressIndex(_ethAddr) {
    const poseidon = await getPoseidon();
    const { F } = poseidon;

    const constant = F.e(constants.SMT_KEY_ADDRESS_INDEX);
...
async function setAddressIndex(ethAddr, index, smt, root) {
    const key = await keyAddressIndex(ethAddr);
...
async function getAddressIndex(ethAddr, smt, root) {
    const key = await keyAddressIndex(ethAddr);
