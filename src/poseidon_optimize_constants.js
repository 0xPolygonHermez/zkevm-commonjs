...
for (let y = 0; y < 12; y++) {
    const row = [];
    for (let x = 0; x < 12; x++) {
        row[x] = Mcirc[(x - y + 12) % 12];
        if (x === y) row[x] += Mdiag[x];
    }
    M.push(row);
}
...
function stringifyBigInts(o) {
    ...
    if ((typeof (o) === 'bigint') || o.eq !== undefined) {
        res = `0x${o.toString(16)}`;
    } else if (o instanceof Uint8Array) {
        res = Scalar.fromRprLE(o, 0);
    } else if (Array.isArray(o)) {
        res = o.map(stringifyBigInts);
...
async function run() {
    // Prime bn128
    const Fr = new ZqField(Scalar.fromString('0xFFFFFFFF00000001'));
    ...
    const oM = matrix_traspose(M);
    const oC = convertConstants(Fr, 12, C, oM);
    const [oP, oS] = calculatePS(Fr, 12, oM);
    opt.C = oC;
    opt.M = oM;
    opt.P = oP;
    opt.S = oS;
    ...
    fs.writeFileSync(path.join('src', 'poseidon_constants_opt.js'), `module.exports = ${JSON.stringify(opt, null, 1)}`, 'utf8');
}
