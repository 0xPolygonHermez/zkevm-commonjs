...
module.exports = class VirtualCountersManager {
    ...
    rlpParsing(input) {
        ...
        this._reduceCounters(Math.ceil(txRLPLength / 136) + 1, 'K');
        ...
        this._reduceCounters(5 * MCP, 'P');
        this._isColdAddress();
        this._mulArith();
        this._addArith();
        this._subArith();
        this._multiCall('_loopBytes', txDataLen);
    }
    ...
    processTx(input) {
        ...
        // Check counters before processing
        this._checkCounter('S', 100000);
        this._checkCounter('B', 100);

        this._reduceCounters(200, 'S');
        this._reduceCounters(12, 'B');
        this._reduceCounters(9 * MCP, 'P');
    ...
    opExtCodeCopy(input) {
        ...
        this._maskAddress();
        this._isColdAddress();
        this._reduceCounters(2 * MCP + Math.ceil(input.bytecodeLen / 56) + 1, 'P');
        this._reduceCounters(Math.ceil(input.bytecodeLen / 56) + 1, 'D');
    ...
    opReturn(input) {
        this._opcode(input);
        this._checkInput(input, ['isCreate', 'isDeploy', 'depth']);
        this._reduceCounters(30, 'S');
        this._reduceCounters(1, 'B');
        this._saveMem();
        if (input.depth > 0) {
    ...
    _hashPoseidonLinearFromMemory(input) {
        ...
        this._reduceCounters(Math.ceil(input.memSize / 56) + 1, 'P');
        this._reduceCounters(Math.ceil(input.memSize / 56) + 1, 'D');
        this._divArith();
    ...
    _ecrecover(input) {
        ...
        // Check counters
        this._checkCounter('S', 6400);
        this._checkCounter('B', 1600);
        this._checkCounter('A', 1100);
    ...
    _processContractCall(input) {
        ...
        if (input.isDeploy || input.isCreate || input.isCreate2) {
            ...
            this._reduceCounters(Math.ceil(input.bytecodeLength / 56) + 1, 'P');
            this._reduceCounters(Math.ceil(input.bytecodeLength / 56) + 1, 'D');
            this._divArith();
        }
    }

    _initBatchProcessing(batchL2DataLength) {
        // 3*MCP + 100S + divArith + batchL2DataLength/136K + K + 1b (skipsetGlobalExitRoot)
        this._reduceCounters(100, 'S');
        this._reduceCounters(3 * MCP, 'P');
        this._divArith();
        this._reduceCounters(Math.ceil(batchL2DataLength / 136) + 1 + 1, 'K');
        this._reduceCounters(1, 'B');
    }
     ...
     * @param {String} counterType identifier
     */
    _checkCounter(counterType, amount = 0) {
        if (this.currentCounters[counterType].amount <= amount) {
