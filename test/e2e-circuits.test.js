...
// 3 blob outers
// A --> 1 blobInner + 3 batchos
// B --> 1 blobInner + 1 batchos
// C --> 1 blobInner + 2 batchos

describe('e2e circuits', function () {
    this.timeout(300000);
    const pathTests = path.join(pathTestVectors, 'end-to-end/e2e-circuits.json');
    let update;
    let geninput;
    let poseidon;
    let F;
        ...
        for (let i = 0; i < testVectors.length; i++) {
            ...
            for (let n = 0; n < blobOuters.length; n++) {
                ...
                for (let k = 0; k < batches.length; k++) {
                    ...
                    if (update && geninput) {
                        const dstFile = path.join(pathInputs, `${path.basename(pathTests, '.json')}-BATCH-${i}-${n}-${k}-input.json`);
                ...
                if (update && geninput) {
                    const dstFileAggBatches = path.join(pathInputs, `${path.basename(pathTests, '.json')}-AGGBATCHES-${i}-${n}-input.json`);
            ...
            if (update && geninput) {
                const dstFileFinal = path.join(pathInputs, `${path.basename(pathTests, '.json')}-FINAL-${i}-input.json`);
